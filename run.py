...
from hybrid.electron import BrowserModule
...
from .hybrid import globals, app
...
def run(*,
    ...
    if show:
        ...
        # if view =='app' and width and height:
        #     reload = False
        #     BrowserModule.browser_open(view=view, url=f'http://{host if host != "0.0.0.0" else "127.0.0.1"}:{port}/', width=width, height=height,title=title)
        # if view =='app' and fullscreen:
        #     reload = False
        #     BrowserModule.browser_open(view=view, url=f'http://{host if host != "0.0.0.0" else "127.0.0.1"}:{port}/', fullscreen=fullscreen,title=title)
    ...
    config = uvicorn.Config(
        'hybrid:app' if reload else globals.app,
        host=host,
        port=port,
        reload=reload,
